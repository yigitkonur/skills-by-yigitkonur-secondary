# Data Case And Tagged Enum
Use `Data.case`, `Data.tagged`, and `Data.taggedEnum` to build structural domain constructors.

## Data.case
`Data.case<T>()` returns a constructor for an immutable structural value of type `T`. Use it when you want a function constructor without creating a class.
...
const makeUser = Data.case<User>()

const a = makeUser({ id: "user_1", name: "Ada" })
const b = makeUser({ id: "user_1", name: "Ada" })

const same = Equal.equals(a, b)
```
...
type AccountEvent = Data.TaggedEnum<{
  Created: { readonly id: string }
  Disabled: { readonly reason: Option.Option<string> }
}>

const { Created, Disabled, $is, $match } = Data.taggedEnum<AccountEvent>()

const describe = (event: AccountEvent): string => {
  if ($is("Created")(event)) {
    return `created:${event.id}`
  }

  return $match(event, {
    Created: (created) => `created:${created.id}`,
    Disabled: (disabled) =>
      Option.match(disabled.reason, {
        onNone: () => "disabled",
        onSome: (reason) => `disabled:${reason}`
      })
  })
}

const first = Created({ id: "user_1" })
const second = Disabled({ reason: Option.some("manual-review") })

const labels = [describe(first), describe(second)]
```

## Narrowing behavior
`$is("Created")(event)` narrows the variant in TypeScript, so `event.id` is available only inside that branch. `$match` is the exhaustive branch point you should prefer when producing a value from every variant.

## When to use classes
Use `Data.TaggedClass` when variants need class constructors, extension, or familiar `new` syntax. Use `Data.taggedEnum` when a closed union and generated constructors are clearer.
...
## Cross-references
See also: [Data.struct](05-data-struct.md), [Either](04-either.md), [Equal and Hash](08-equal-and-hash.md).
