# DateTime
Use `DateTime` for UTC and zoned time values instead of carrying raw `Date` through domain code.

## Version and shape
`DateTime` is available in Effect v3.6.0 and later. In v3.21.2 it has two variants: `Utc` and `Zoned`. `Utc` is a point in time; `Zoned` keeps the point plus a time zone.

## Construction
```typescript
import { DateTime, Option } from "effect"

const fromIso = DateTime.make("2024-01-02T03:04:05.000Z")

const label = Option.match(fromIso, {
  onNone: () => "invalid",
...
const zoned = DateTime.makeZoned("2024-01-02T03:04:05.000Z", {
  timeZone: DateTime.zoneUnsafeMakeNamed("America/New_York")
})

const output = Option.map(zoned, DateTime.formatIsoOffset)
```
...
const maybeStart = DateTime.make("2024-01-02T03:04:05.000Z")

const shifted = Option.map(maybeStart, (start) =>
  DateTime.addDuration(start, "5 minutes")
)
```

## Math without invalid Option pipe
```typescript
import { DateTime, Option, pipe } from "effect"

const shifted = pipe(
  DateTime.make("2024-01-02T03:04:05.000Z"),
  Option.map((start) => DateTime.add(start, { days: 1, hours: 2 })),
  Option.map((start) => DateTime.startOf(start, "day")),
  Option.map((start) => DateTime.endOf(start, "day"))
)
```

## Boundary conversions
Use `DateTime.make` for safe construction from string, number, partial parts, or date input. `DateTime.unsafeFromDate` and `DateTime.unsafeMake` exist for trusted edges, but domain constructors should prefer the safe functions returning `Option`.

Use `DateTime.toDate` or `DateTime.toDateUtc` only at interop edges that require a JavaScript `Date`.

## Formatting
Use `DateTime.formatIso` for stable UTC ISO output. Use `DateTime.formatIsoOffset` or `DateTime.formatIsoZoned` for zoned values when offset or zone identity matters. Use `DateTime.format` when human-facing locale formatting is the goal.
...
## Cross-references
See also: [Duration](12-duration.md), [Option](02-option.md), [Order and Equivalence](17-order-and-equivalence.md).
