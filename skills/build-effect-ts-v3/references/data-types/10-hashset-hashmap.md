# HashSet And HashMap
Use structural collections when membership and keys should respect Effect equality.

## Why not Set and Map everywhere
JavaScript `Set` and `Map` use reference identity for objects. `HashSet` and `HashMap` use Effect equality and hashing, so `Data` values with the same fields behave as the same member or key.

## HashSet membership
```typescript
import { Data, HashSet } from "effect"

const first = Data.struct({ tenant: "a", id: "1" })
const second = Data.struct({ tenant: "a", id: "1" })

const set = HashSet.make(first)
const hasSecond = HashSet.has(set, second)
```
...
const key = Data.struct({ tenant: "a", id: "1" })
const sameKey = Data.struct({ tenant: "a", id: "1" })

const map = HashMap.make([key, "cached-value"])

const rendered = Option.match(HashMap.get(map, sameKey), {
  onNone: () => "missing",
  onSome: (value) => value
})
```
...
const original = HashMap.make(["a", 1])
const updated = HashMap.set(original, "b", 2)
const removed = HashMap.remove(updated, "a")
```
...
const numbers = HashSet.make(1, 2, 3, 4)
const evens = HashSet.filter(numbers, (n) => n % 2 === 0)

const map = HashMap.make(["a", 1], ["b", 2])
const doubled = HashMap.map(map, (value) => value * 2)
```

## Rules
- Use `Data` values as keys when key identity is structural.
- Use ordinary `Map` only when reference identity is intended or interop requires it.
- Convert with `HashMap.toEntries` or `HashSet.toValues` at output boundaries.
- Do not mutate objects used as keys after inserting them.
...
- Keep collection ordering explicit when output stability matters.
## Cross-references
See also: [Data.struct](05-data-struct.md), [Equal and Hash](08-equal-and-hash.md), [Chunk](09-chunk.md), [ReadonlyArray](11-readonly-array.md).
