# Chunk
Use `Chunk` for immutable Effect-native sequences, and know when a plain readonly array is enough.

## Chunk vs Array
`Chunk.Chunk<A>` is an immutable sequence data type with structural equality and persistent operations. A readonly array is still the right choice for small fixed sequences, framework APIs, and hot paths where native array behavior is exactly what you need.

Use `Chunk` when the sequence is part of an Effect-facing API, will be appended/split/transformed repeatedly, or should compare structurally. Use `ReadonlyArray` plus the `Array` module when you are sorting, slicing, or adapting ordinary JavaScript arrays.

## Construct and convert
```typescript
import { Chunk } from "effect"

const values = Chunk.fromIterable([1, 2, 3])
const more = Chunk.append(values, 4)
const backToArray = Chunk.toReadonlyArray(more)
```
...
const firstLabel = (values: Chunk.Chunk<string>): string =>
  Option.match(Chunk.head(values), {
    onNone: () => "empty",
    onSome: (value) => value
  })
```
...
const numbers = Chunk.make("1", "x", "3")

const parsed = pipe(
  numbers,
  Chunk.filterMap((value) => {
...
const input = Chunk.range(1, 10)
const batches = Chunk.chunksOf(input, 3)
const firstFive = Chunk.take(input, 5)
const withoutFirstTwo = Chunk.drop(input, 2)
```

## When Array is better
Prefer readonly arrays when:

- The value is already an array from a framework and you only need one pass.
- Sorting with `Arr.sort`, `Arr.sortBy`, or `Arr.sortWith` is the central operation.
- The array is a small literal list of fields, routes, or options.
- A third-party API requires an array and no Effect collection semantics are needed.

## When Chunk is better
Prefer `Chunk` when:

- You expose an immutable sequence from an Effect service.
- Structural equality of the sequence matters.
- You repeatedly append, prepend, split, or batch.
- You want safe accessors returning `Option`.
- You want a clear signal that callers should not mutate the sequence.
...
- Document the boundary where protected values are unwrapped.
## Cross-references
See also: [ReadonlyArray](11-readonly-array.md), [Option](02-option.md), [Data tuple and array](06-data-tuple-array.md), [HashSet and HashMap](10-hashset-hashmap.md).
