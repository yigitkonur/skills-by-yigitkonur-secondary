# Redacted
Use `Redacted` for secrets so inspection, JSON, and logging do not reveal raw values.

## What Redacted protects
`Redacted.Redacted<A>` stores a sensitive value behind a protected wrapper. Its string, inspection, and JSON representations are redacted. In v3 source, `toString`, node inspection, and `toJSON` all return the redacted marker rather than the underlying value.

## Create redacted values
```typescript
import { Redacted } from "effect"

const apiKey = Redacted.make("secret-value")
const printable = String(apiKey)
const json = JSON.stringify(apiKey)
```

## Config unwrap pattern
Keep secrets redacted until the boundary that truly needs the raw string. For config, unwrap with `Redacted.value(yield* config)` inside the effect that opens the external connection.

## Unwrap at the boundary
```typescript
import { Config, Effect, Redacted } from "effect"

declare const connect: (url: string) => Effect.Effect<void>

const program = Effect.gen(function* () {
  const config = Config.redacted("DATABASE_URL")
  const databaseUrl = Redacted.value(yield* config)
  return yield* connect(databaseUrl)
})
```
...
## Rules
- `Redacted` should enter the system immediately after loading a secret.
- Do not store raw secret strings in domain records.
- Do not unwrap in helper functions just to make typing easier.
- `JSON.stringify` does not expose the underlying value.
- `Effect.log` receives the redacted representation, not the secret.
- Use `Redacted.value` only at the IO boundary that requires the real value.
- `Redacted.unsafeWipe` exists for explicit erasure, but normal application code rarely needs it.
...
- Import data modules from the `effect` barrel.
## Cross-references
See also: [Option patterns](03-option-patterns.md), [Data.struct](05-data-struct.md), [Function utilities](18-function-utilities.md).
