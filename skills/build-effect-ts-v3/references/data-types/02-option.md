# Option
Use `Option` for absence that is expected, typed, and explicitly handled.

## Core constructors
`Option.Option<A>` is either `Some<A>` or `None`. It replaces nullable domain fields and unchecked array access.

The most common constructors are:

- `Option.some(value)` for present values.
- `Option.none()` for absent values.
- `Option.fromNullable(value)` at boundaries where input may be nullable.
- `Option.liftNullable(fn)` when wrapping an existing nullable function.
- `Option.liftPredicate(predicate)` when filtering into a typed optional value.
...
const fromApi = (input: { readonly id: string; readonly email: string | null }): User => ({
  id: input.id,
  email: Option.fromNullable(input.email)
})

const label = (user: User): string =>
  pipe(
    user.email,
    Option.map((email) => email.toLowerCase()),
    Option.getOrElse(() => "missing-email")
  )
```

## Match instead of extracting
Do not use unchecked extraction for normal control flow. `Option.getOrThrow` is forbidden in this skill because absence is supposed to be part of the type. Use `Option.match` or `Option.getOrElse`.

Anti-pattern route: [option get-or-throw](../anti-patterns/11-option-getorthrow.md).

## Match example
```typescript
import { Option } from "effect"

const renderEmail = (email: Option.Option<string>): string =>
  Option.match(email, {
    onNone: () => "No email on file",
    onSome: (value) => `Email: ${value}`
  })
```
...
const describePort = (value: string): string =>
  pipe(
    parsePort(value),
    Option.filter((port) => port < 65_536),
...
const makeDisplayName = (
  ...
  Option.zipWith(first, last, (a, b) => `${a} ${b}`)

const profile = Option.all({
  id: Option.some("user_1"),
  email: Option.some("a@example.com")
})
```

## Boundary discipline
Read [Option patterns](03-option-patterns.md) next. This file covers the module mechanics; the next file covers where nullable values are allowed and where they are not.
...
- Document the boundary where protected values are unwrapped.
## Cross-references
See also: [Option patterns](03-option-patterns.md), [Either](04-either.md), [ReadonlyArray](11-readonly-array.md), [Anti-pattern](../anti-patterns/11-option-getorthrow.md).
