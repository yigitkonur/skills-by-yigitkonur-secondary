# Data Struct
Use `Data.struct` for immutable records with structural equality and hashing.

## Why Data.struct exists
Plain object equality in JavaScript is reference equality. `Data.struct` returns an immutable structural value implementing Effect equality and hashing. Two structs with the same fields compare equal with `Equal.equals` even when they were created separately.

## Structural equality
```typescript
import { Data, Equal } from "effect"

const first = Data.struct({ id: "user_1", count: 2 })
const second = Data.struct({ id: "user_1", count: 2 })

const same = Equal.equals(first, second)
```
...
const makeUser = (input: { readonly id: string; readonly email: string | null }): User =>
  Data.struct({
    id: input.id,
...
Use `Data.Class` when you want a named constructor and `instanceof` behavior. Use `Data.struct` when a simple value constructor is enough and a class would add ceremony.
...
const hasEquivalent = HashSet.has(set, b)
```

## Rules
- Treat `Data.struct` values as domain values, not DTOs with hidden behavior.
- Prefer readonly fields.
- Store `Option` for optional domain fields.
- Use `DateTime` for time fields instead of `Date`.
- Use `Data.Class` or `Data.TaggedClass` when a named type is clearer.
...
- Make invalid states unrepresentable where the module supports it.
## Cross-references
See also: [Data tuple and array](06-data-tuple-array.md), [Data case](07-data-case.md), [Equal and Hash](08-equal-and-hash.md), [HashSet and HashMap](10-hashset-hashmap.md).
