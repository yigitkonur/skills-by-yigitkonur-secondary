# Data Tuple And Array
Use `Data.tuple` and `Data.array` when sequence values need structural equality.

## Tuple values
`Data.tuple` creates a readonly tuple with structural equality. Use it for small product values where each position has meaning.

## Tuple example
```typescript
import { Data, Equal } from "effect"

const a = Data.tuple("user_1", 1)
const b = Data.tuple("user_1", 1)

const same = Equal.equals(a, b)
```

## Array values
`Data.array` copies the input array and gives the resulting readonly array structural equality. Use it when the whole sequence is itself a domain value.

## Array example
```typescript
import { Data, Equal } from "effect"

const left = Data.array(["read", "write"])
const right = Data.array(["read", "write"])

const samePermissions = Equal.equals(left, right)
```

## Data.array vs Chunk
`Data.array` is for a structural data value. `Chunk` is for an immutable sequence API with many collection operations. If callers will map, filter, append, split, or stream the sequence, prefer `Chunk`. If the sequence is a field in a structural record, `Data.array` is often enough.

## Rules
- Do not use `Data.unsafeArray` unless you have already isolated mutation risk.
- Prefer `Data.tuple` for fixed arity values.
- Prefer `Data.array` for structural equality of a sequence value.
- Prefer `Chunk` for collection-heavy APIs.
- Convert to ordinary arrays only at library boundaries that require them.
...
- Keep collection ordering explicit when output stability matters.
## Cross-references
See also: [Data.struct](05-data-struct.md), [Data case](07-data-case.md), [Chunk](09-chunk.md).
